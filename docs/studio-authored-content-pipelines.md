...
may own advanced challenge semantics in Rust, compact TypeScript may compile to
a closed `behaviorPackage`, and browser presentation code may own HUD layout.
Studio need not become a source-code editor. It must, however, expose the stored
package or selected provider/resource identity and all authored configuration
that changes how the compiled behavior is used.
...
| Prefab definitions and variants | The proof-only console registry was removed; the security switch now earns a real prefab role | **Stored when used:** a project's canonical prefab registry remains its single definition/variant/part-role source | Existing Rust/public prefab validation; Studio prefab/variant inspector | Rust resolves the nearest eligible `interaction/switch` part. #6091 |
| Prefab instances and overrides | The proof-only blue/red placements and overrides were removed | **Stored when used:** scene `entityInstance` prefab nodes are the only placement source and authored signals target stable scene instance and role refs | Rust scene/prefab/variant/behavior reference validation; Studio scene placement and package navigation | The stored security-switch placement participates in authoritative interaction resolution. #6091 |
| Gameplay implementation | `demo-rs/crates/primary-fire-effect` owns advanced challenge reactions; `src/content/security-door.ts` owns compact data-only door intent | **Stored:** provider/module binary for the advanced lane and canonical `behaviorPackage` JSON for ordinary bounded behavior | Rust Fabric provider owns challenge semantics; Engine project admission privately compiles the package to closed authority verbs | Runtime invokes the composed provider beside the direct authored-program path. #5950 and #6091 |
...
| Player prefab interaction | The deleted bootstrap shortcut used to fire a fixed event during backend creation | **Runtime:** `demo.interact` submits actor, role, distance, expected runtime hash, and tick; Rust resolves the authoritative nearby part | Studio authors the switch prefab role and behavior signal; the browser only submits the named action | The compiled authored program reacts directly; Fabric remains available for subscribed advanced modules. #6088 and #6091 |
| Animated mesh resource | The GLB and license are manifest resources referenced by the typed presentation catalog; there is no second renderer-only manifest file | **Stored:** asset bytes/license plus the canonical ProjectContent resource entry | Rust validates asset kind, path, content hash, clips, and manifest closure; Studio navigates the resource | TypeScript derives the renderer-host descriptor from Rust-admitted ProjectContent. #6060 |
| Animation/audio/particle cues | `catalogs/presentation/catalog.json` stores sampled timing, semantic signal ids, resource bindings, gain, and scale; WAV/SVG are ordinary manifest resources | **Stored:** presentation-cue/catalog records and real resource files with canonical identities | Rust validates resource kinds/hashes and signal closure; Studio edits bounded typed cue fields | Renderer/audio/particle hosts realize only Rust-admitted resources; missing bytes visibly degrade. #6060 |
...
| Launch camera and controller settings | `project-content.ts` previously supplied camera pose/projection, collision extents, solver iterations, and grounded mode as browser literals | **Stored:** provider-owned `demo.launch-settings.default` selects the player EntityDefinition and owns projection, movement mode, collision extents, and solver iterations. A typed scene marker owns initial pose | The statically composed `demo.launch-settings` Rust provider owns the codec and cross-field validation; Studio's gameplay-configuration inspector renders its typed fields and the ordinary scene inspector edits the marker | A fresh RuntimeSession admits the configuration, materializes the referenced actor from the stored marker, and only then lets the Demo create its transient camera. #6059 |
...
`sessionId` and the seed passed when creating a new RuntimeSession are
operational launch identity, not durable game-content tuning. They stay in the
Demo host because changing them does not change the authored camera,
controller, actor, or scene seen by another project consumer. All values which
do change that behavior are in the Rust-admitted configuration or scene.
...
- opaque hand-maintained configuration byte arrays and build-derived digests;
  and
- any future presentation literal that duplicates a typed cue or resource in
  `catalogs/presentation/catalog.json`.
...
The material and presentation items in that inventory were completed by
#6060. Normal play reads the Rust-admitted catalogs, verifies resource bytes,
and never regenerates or writes them. The retained offline WAV generator is an
explicit content-authoring tool; it is not invoked by build, load, or play.
